...
import { book, stories } from './batch-012-data.mjs';
...
const paths = {
  ...
  batch: resolve(root, 'HIKAYE_URETIM_BATCH_012.md'),
};

const STORY_VERSION = 'F6';
const LANGS = ['tr', 'en', 'es', 'de'];
const categoryNames = {
  16: { tr: 'Sağlık', en: 'Health', es: 'Salud', de: 'Gesundheit' },
};
const categoryLabelTr = { 16: 'Sağlık' };

const countWords = (t) => t.trim().split(/\s+/u).filter(Boolean).length;

// --- Ön doğrulama ---
// Diller arası adil ölçüt karakter sayısıdır (Türkçe sondan eklemeli olduğu için
// aynı anlamı daha az kelimeyle taşır). İçerik ~1050–1600 karakter (V1 medyanının
// belirgin üstü), 30 sn özet 300–560 karakter; ayrıca V1 işaretleri mevcut olmalı.
const violations = [];
for (const s of stories) {
  for (const lang of LANGS) {
    ...
    if (cc < 1050 || cc > 1600) violations.push(`${s.key}/${lang}: content ${cc}ch (${countWords(d.content)}w)`);
...
for (const lang of LANGS) {
  ...
  if (bt?.values?.length) db.run('UPDATE book_translations SET title=?, category_name=? WHERE book_id=? AND lang_code=?', [book.title, categoryName, bookId, lang]);
  else db.run('INSERT INTO book_translations (book_id, lang_code, title, category_name) VALUES (?, ?, ?, ?)', [bookId, lang, book.title, categoryName]);
}
...
const rowAfter = `| ${book.catalogNo} | *${book.title}* | ${book.author} | ${label} | **DB'DE KAYITLI / F6** | ${book.listNo} | ${stories.length} |`;
if (catalog.includes(rowBefore)) {
  catalog = catalog.replace(rowBefore, rowAfter);
  // başlık sayaçları: 272 kitap / 28 yeni öneri -> 273 kitap / 27 yeni öneri
  catalog = catalog.replace(
    'uygulama veritabanındaki 272 kitabı ve içerik üretimi için seçilen 28 yeni kitabı',
    'uygulama veritabanındaki 273 kitabı ve içerik üretimi için seçilen 27 yeni kitabı');
...
// --- Batch dokümanı ---
const lines = [
  '# Hikâye Üretim Batch 012',
  '',
  `- **Kitap:** *${book.title}* — ${book.author} (${label}, katalog no ${book.catalogNo}, DB list_no ${book.listNo})`,
  '- **Version:** F6 (Claude Fable 5 ile üretildi)',
  '- **Format:** V1 çok paragraflı; ## anahtar iç görü ##, $$ ders $$, && soru && işaretleri.',
  '- **Kelime hedefi:** 220–280 (içerik), 55–80 (30 sn özet)',
