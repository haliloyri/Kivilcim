...
import { book, stories } from './batch-014-data.mjs';
...
const paths = {
  ...
  batch: resolve(root, 'HIKAYE_URETIM_BATCH_014.md'),
};

const STORY_VERSION = 'C1';
...
const rowAfter = `| ${book.catalogNo} | *${book.title}* | ${book.author} | ${label} | **DB'DE KAYITLI / C1** | ${book.listNo} | ${stories.length} |`;
...
const lines = [
  '# Hikâye Üretim Batch 014',
  '',
  `- **Kitap:** *${book.title}* — ${book.author} (${label}, katalog no ${book.catalogNo}, DB list_no ${book.listNo})`,
  '- **Version:** C1 (zengin okuma formatı)',
  '- **Format:** F7+; `~~önce :: sonra~~` zıtlık bloğu, görünür `$$ders$$` ve dokunulabilir `&&soru&&`.',
